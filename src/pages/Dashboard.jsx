...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (profile && (!profile.phone || !profile.address || !profile.purok)) {
      ...
      {showProfileSetupModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[10000] p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="sticky top-0 bg-white p-4 md:p-5 border-b border-gray-100 z-10">
              <div className="flex items-center gap-2">
                <User size={20} className="text-blue-600" />
                <h3 className="text-base md:text-lg font-semibold text-gray-900">Complete Your Profile</h3>
              </div>
              <p className="text-xs text-gray-500 mt-1">Please provide the following information to continue</p>
            </div>

            {/* Modal Body */}
            <form onSubmit={handleCompleteProfile} className="p-4 md:p-6">
              {profileError && (
              ...
              <div className="mt-6">
                <button
                  type="submit"
                  className="w-full px-4 py-3 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  disabled={savingProfile}
                >
                  {savingProfile ? (
                    'Saving...'
              ...
              <div className="mt-3 text-center">
                <p className="text-xs text-gray-500">
                  ⚠️ This information is required to use the app
                </p>
              </div>
            </form>
          </div>
